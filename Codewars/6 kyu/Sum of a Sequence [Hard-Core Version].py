...
def sequence_sum(begin_number, end_number, step):
    if (begin_number > end_number and step > 0) or \
            (end_number > begin_number and step < 0):
        return 0
    n = (end_number - begin_number) // step + 1
    if (n - 1) * step + begin_number > end_number and step > 0:
        n -= 1
    elif (n - 1) * step + begin_number < end_number and step < 0:
        n -= 1
