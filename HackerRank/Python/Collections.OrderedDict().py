...
for _ in range(int(input())):
    name, price = (lambda x: [' '.join(x[:-1]), int(x[-1])])(input().split())
    prices[name] = prices.get(name, price)
    quantity[name] = quantity.get(name, 0) + 1
ans = [print(*[name, prices[name] * quantity[name]]) for name, price in quantity.items()]
